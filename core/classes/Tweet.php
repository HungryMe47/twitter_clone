...
class Tweet extends User {
    ...
    protected static $pdo;
      ...
      public static function tweets($user_id) {
        ...
        WHERE user_id = :user_id OR user_id IN (SELECT following_id from `follow` WHERE follower_id = :user_id) OR user_id IN (SELECT user_id FROM posts WHERE id IN (SELECT post_id FROM likes WHERE user_id = :user_id))
      ...
      public static function getUserTweetsFromLikedTweets($user_id) {
        // Step 1: Get the list of users whose tweets you've liked
        $stmt = self::connect()->prepare("SELECT DISTINCT user_id FROM likes WHERE user_id != :user_id AND post_id IN (SELECT post_id FROM likes WHERE user_id = :user_id)");
        $stmt->bindParam(":user_id", $user_id, PDO::PARAM_STR);
        $stmt->execute();
        $liked_users = $stmt->fetchAll(PDO::FETCH_COLUMN);
    
        // Step 2: Retrieve all tweets from the selected users
        $user_tweets = array();
        foreach ($liked_users as $liked_user_id) {
            $stmt = self::connect()->prepare("SELECT * FROM posts WHERE user_id = :liked_user_id ORDER BY post_on DESC");
            $stmt->bindParam(":liked_user_id", $liked_user_id, PDO::PARAM_STR);
            $stmt->execute();
            $user_tweets[$liked_user_id] = $stmt->fetchAll(PDO::FETCH_OBJ);
        }
